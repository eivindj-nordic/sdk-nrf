...
static void app_download_btn_irq_disable(void)
{
	gpio_pin_interrupt_configure_dt(&sw0, GPIO_INT_DISABLE);
}

static void app_download_btn_irq_enable(void)
{
	gpio_pin_interrupt_configure_dt(&sw0, GPIO_INT_EDGE_TO_ACTIVE);
}

static void app_download_button_pressed(const struct device *gpiob, struct gpio_callback *cb,
			uint32_t pins)
{
	int err;
...
static int button_init(void)
{
	...
	gpio_init_callback(&sw0_cb, app_download_button_pressed, BIT(sw0.pin));
...
static int apply_state(enum fota_state new_state)
{
	...
	switch (new_state) {
	case IDLE:
		app_download_btn_irq_disable();
		modem_configure_and_connect();
		break;
	case CONNECTED:
		app_download_btn_irq_enable();
